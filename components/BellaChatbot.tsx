...
import {
  ...
  Keyboard, Animated, Dimensions, KeyboardAvoidingView,
} from 'react-native';
...
const renderFormattedText = (text: string) =>
  text.split(/(\*\*.*?\*\*)/g).map((part, i) =>
    part.startsWith('**') && part.endsWith('**')
      ? <Text key={i} style={{ fontWeight: '900', color: '#000' }}>{part.slice(2, -2)}</Text>
...
const BellaChatbot: React.FC = () => {
  ...
  useEffect(() => {
    Animated.spring(tooltipAnim, { toValue: 1, tension: 40, friction: 7, useNativeDriver: true }).start();
  }, []);

  // ─── Track keyboard height manually — no jitter ───────────────────────────
  useEffect(() => {
  ...
  const send = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const reply = await sendMessageToChatbot(history, text);
      setMsgs(prev => [...prev, { id: (Date.now()+1).toString(), role: 'bot', text: reply, time: getTime() }]);
      setHistory(prev => {
      ...
      setMsgs(prev => [...prev, {
        id: (Date.now()+1).toString(), role: 'bot', time: getTime(),
  ...
  // Dialog height adjusts based on keyboard — no KAV needed
  const dialogMaxHeight = keyboardHeight > 0
    ? SCREEN_H - keyboardHeight - 60
    : SCREEN_H * 0.78;
    ...
    <>
      {!isOpen && (
        <View style={styles.fabContainer}>
          ...
          <TouchableOpacity onPress={() => setIsOpen(true)} activeOpacity={0.8} style={styles.fabTouchable}>
            <View style={styles.fabInner}><BellaIcon size={46} /></View>
          </TouchableOpacity>
        </View>
      )}
      ...
      >
        ...
        {/* Dialog — positioned from bottom, moves up with keyboard */}
        <View style={[
          styles.dialogWrapper,
          { bottom: keyboardHeight > 0 ? keyboardHeight + 16 : SCREEN_H * 0.1 }
        ]}>
          <View style={[styles.dialogCard, { maxHeight: dialogMaxHeight }]}>
...
const styles = StyleSheet.create({
  ...
  tooltip: {
    backgroundColor: C.textPrim, paddingHorizontal: 12, paddingVertical: 8,
  ...
  tooltipArrow: {
    ...
    width: 10, height: 10, backgroundColor: C.textPrim,
  ...
  input: {
    ...
    fontSize: 14, color: C.textPrim, maxHeight: 100,
    borderWidth: 1, borderColor: C.border,
  },
