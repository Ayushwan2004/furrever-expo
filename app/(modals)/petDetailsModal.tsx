/**
 * PetDetailsModal.tsx  v2
 * ─────────────────────────────────────────────────────────────────────────────
 * Changes in v2:
 *   • InfoCard (Breed / Color) uses flexWrap + adjustsFontSizeToFit so long
 *     words never get clipped — they wrap or shrink to fit.
 *   • Phone number row shown in the grid (below Breed/Age/Color) when the
 *     owner has set displayPhone = true.  Tapping opens the dialler.
 *   • Minor: "Meet the Owner" section also shows a call button if the owner
 *     has display_phone set (owner can call themselves is guarded).
 */
import React, {
...
import {
  ArrowClockwise, Calendar, CaretRight, ChatCircleDots,
  CheckCircle, Envelope, Handshake, Info, MapPin,
...
// ─── InfoCard — handles long text gracefully ──────────────────────────────────
/**
 * Long breed names ("Pembroke Welsh Corgi") or coat colours ("Chocolate & Tan")
 * used to get truncated. Fixed by:
 *  1. Removing numberOfLines limit — text wraps to a second line if needed.
 *  2. `adjustsFontSizeToFit` + `minimumFontScale` as a fallback for very long
 *     single words that can't wrap (rare but can happen with user input).
 *  3. `flexShrink: 1` on the value text container prevents the card from
 *     exploding in width.
 */
...
// ─── Phone call helper ────────────────────────────────────────────────────────
function openDialler(phone: string) {
  const tel = `tel:${phone.replace(/\s/g, "")}`;
  Linking.canOpenURL(tel).then(ok => {
    if (ok) Linking.openURL(tel);
    else Alert.alert("Unable to open dialler", "Please dial manually: " + phone);
  });
}
...
const PetDetailsModal = () => {
  ...
  // ── Real-time sync ────────────────────────────────────────────────────────
  useEffect(() => {
    if (!currentUser || !pet?.ownerId) { setFetchingData(false); return; }
  ...
  const handleCancelRequest = useCallback(() => {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: "Yes, Cancel", style: "destructive", onPress: () => handleThrottledAction(() => cancelApplication(userApplication.id)) },
      ],
    );
  ...
  // ── Phone visibility ──────────────────────────────────────────────────────
  // `pet.displayPhone` is a boolean stored on the pet document.
  // `pet.phone` is the owner's contact number.
  // We show the phone row only when both are truthy AND the viewer is not the owner
  // (owners can see their own number from their profile).
  const showPhone = !isOwner && !!(pet as any)?.displayPhone && !!(pet as any)?.phone;
  ...
  const adoptButtonSection = useMemo(() => {
    ...
    if (isOwner) return (
      <View style={styles.ownerListingInfo}>
        ...
        <Typo color={colors.primary} fontWeight="700">You listed this buddy</Typo>
      </View>
    );
    ...
    if (applicationStatus === 'pending') return (
      <TouchableOpacity style={[styles.statusButton, styles.cancelBtn]} onPress={handleCancelRequest}>
  ...
  return (
    <ScreenWrapper style={styles.container}>
      <View style={styles.headerContainer}>
        <Header title="Buddy Details" leftIcon={<BackButton />} />
      </View>
        ...
        <View style={styles.imageContainer}>
          ...
          {isSold && (
            <View style={styles.soldBadge}>
              <SealCheck size={20} color="white" weight="fill" />
              <Typo color="white" fontWeight="700">ADOPTED</Typo>
            </View>
          )}
        </View>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.topMeta}>
            <View style={styles.certifiedBadge}>
              ...
              <Text style={styles.certifiedText}>Verified Listing</Text>
            </View>
          ...
          <View style={styles.titleRow}>
            <View style={{ flex: 1 }}>
              ...
              <TouchableOpacity
                ...
                onPress={() => Linking.openURL(`geo:0,0?q=${encodeURIComponent(pet.address)}`)}
              >
            ...
            <View style={styles.categoryBox}>
              <Typo color={colors.primary} fontWeight="700" size={13}>{pet.category}</Typo>
            </View>
          </View>
          ...
          <View style={styles.grid}>
            <InfoCard
              icon={<PawPrint size={22} color={colors.primary} weight="duotone" />}
              label="Breed"
              value={pet.breed}
            />
            <InfoCard
              icon={<Calendar size={22} color={colors.primary} weight="duotone" />}
              label="Age"
              value={pet.age ? `${pet.age} Yrs` : 'Baby'}
            />
            <InfoCard
              icon={<Palette size={22} color={colors.primary} weight="duotone" />}
              label="Color"
              value={(pet as any).coatcolor || 'Mix'}
            />
          </View>

          {/* Phone number row — shown only when owner opts in */}
          {showPhone && (
            <TouchableOpacity
              style={styles.phoneRow}
              activeOpacity={0.75}
              onPress={() => openDialler((pet as any).phone)}
            >
              <View style={styles.phoneIconWrap}>
                <Phone size={20} color={colors.green} weight="fill" />
              </View>
              <View style={{ flex: 1 }}>
                <Typo size={11} color={colors.textLighter} fontWeight="600">Owner's Phone</Typo>
                <Typo size={15} fontWeight="700" color={colors.text}>{(pet as any).phone}</Typo>
              </View>
              <View style={styles.callChip}>
                <Typo size={12} color={colors.green} fontWeight="700">Call</Typo>
              </View>
            </TouchableOpacity>
          )}

          {/* Description */}
          <Typo size={18} fontWeight="700" style={{ marginBottom: 10, marginTop: 10 }}>Description</Typo>
          <Typo color={colors.textLight} style={styles.descText}>
            {pet.description || "No specific details provided."}
          </Typo>
          ...
          <View style={styles.ownerSection}>
            ...
            <TouchableOpacity
              ...
              onPress={() => handleThrottledAction(() => {
                if (isOwner) router.push("/(tabs)/profile");
                else router.push({ pathname: "/(modals)/userAnalyticsModal", params: { userId: pet.ownerId! } });
              })}
            >
              <Image
                source={ownerData?.image ? { uri: ownerData.image } : require('../../assets/Avatar.jpg')}
                style={styles.ownerAvatar}
                cachePolicy="memory-disk"
              />
              <View style={styles.ownerDetails}>
                <Typo fontWeight="700" size={16}>
                  {ownerData?.name || (fetchingData ? "Loading…" : "Pet Owner")}
                </Typo>
                <View style={styles.roleBadge}>
                  <Typo size={10} color={colors.primary} fontWeight="800">
                    {ownerData?.role?.toUpperCase() || 'MEMBER'}
                  </Typo>
                </View>
              </View>

              {/* Action buttons on owner card */}
              {!isOwner && !isSold && (
                <View style={styles.ownerActions}>
                  {/* Chat */}
                  <TouchableOpacity
                    style={styles.ownerActionBtn}
                    onPress={(e) => { e.stopPropagation(); handleChatPress(); }}
                  >
                    <ChatCircleDots size={22} color={colors.primary} weight="fill" />
                  </TouchableOpacity>

                  {/* Call — only if owner opted to display phone */}
                  {showPhone && (
                    <TouchableOpacity
                      style={[styles.ownerActionBtn, styles.ownerCallBtn]}
                      onPress={(e) => { e.stopPropagation(); openDialler((pet as any).phone); }}
                    >
                      <Phone size={22} color={colors.green} weight="fill" />
                    </TouchableOpacity>
                  )}
                </View>
              )}
...
const styles = StyleSheet.create({
  ...
  gridItem: {
    ...
    // Allow the card height to grow if text wraps to 2 lines
  ...
  gridValue: {
    ...
    color: colors.text,
    textAlign: 'center',
    // flexShrink prevents the card expanding beyond its 1/3 slot
    flexShrink: 1,
    width: '100%',
  },
  ...
  footer: {
    ...
    paddingBottom: Platform.OS === 'ios' ? spacingY._35 : spacingY._20,
