...
export const AdoptionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const sendApplication = useCallback(async (pet: PetType): Promise<ResponseType> => {
    if (!user?.uid) return { success: false, msg: "Login required" };
    setLoading(true);
    try {
      // Get fresh user name from Firestore in case it's not in context yet
      let adopterName = user.name || user.email || "User";
      try {
        const userSnap = await getDoc(doc(firestore, "users", user.uid));
        if (userSnap.exists()) {
          adopterName = userSnap.data().name || adopterName;
        }
      } catch { }
      ...
      await addDoc(collection(firestore, "notifications"), {
        ...
        message: `${adopterName} wants to adopt ${pet.name}!`,
        type: "new_request",
        isRead: false,
        createdAt: serverTimestamp(),
      });
      ...
      if (ownerToken) {
        await sendExpoPush(
          ...
          `${adopterName} wants to adopt ${pet.name}!`,
  ...
  const cancelApplication = useCallback(async (appId: string): Promise<ResponseType> => {
    setLoading(true);
    try {
  ...
  const updateApplicationStatus = useCallback(async (
    ...
    try {
      ...
      batch.update(doc(firestore, "adoptions", appId), {
        status,
      ...
      if (status === "approved") {
        ...
        batch.update(doc(firestore, "users", app.adopterId), {
      ...
      const notifTitle = status === "approved" ? "Adoption Approved! 🎉" : "Application Update";
  ...
  }), [applications, loading, sendApplication, cancelApplication, updateApplicationStatus]);
